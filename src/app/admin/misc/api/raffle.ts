...
const pickBall = async (data: IBallPickData) => {
  const res = await tokenlessAxios.post(
    "api/admin-controller/hustle_match/",
    data
  );
...
const goldenBallPick = async (data: IBallPickData) => {
  const res = await tokenlessAxios.post(
    "api/admin-controller/golden_hustle_match",
    data
  );
  return res.data as BallPickAPIResponse;
};
...
export const getHustleMatches = async (gameEpisode: number) => {
  ...
  const response = await tokenlessAxios.get(
    `api/admin-controller/get_hustle_matches?game_episode=${gameEpisode}`
  );
...
export const getMatchedHustles = async (gameEpisode: number) => {
  if (!gameEpisode) return null;
  const response = await tokenlessAxios.get(
    `/api/admin-controller/get_matched_hustles?game_episode=${gameEpisode}`
  );
  return response?.data as MatchedHustlesResponse;
};

export const useGetMatchedHustles = (
  gameEpisode: number,
  slowRefetch?: boolean
) =>
  useQuery({
  ...
  type GoldeMatchAmount = {
    amount: number,
    stage_four: string
  }
const getGoldenMatchAmount = async (gameEpisode: number) => {
  const response = await tokenlessAxios.get(
    `api/admin-controller/get_golden_match_amount?game_episode=${gameEpisode}`
  );
  return response.data as GoldeMatchAmount;
};

export const useGetGoldenMatchWinAmount = (gameEpisode: number) => {
  return useQuery({
    queryFn: () => getGoldenMatchAmount(gameEpisode),
    queryKey: ["golden-match-amount", gameEpisode],
  });
};
