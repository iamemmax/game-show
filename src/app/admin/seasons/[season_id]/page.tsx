...
export default function EpisodesPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetAllSeasonEpisodes(
    season_id ? { season_id: season_id as string | number } : {}
  );
  ...
  const filteredEpisodes = useMemo(() => {
    return (
      episodesResponse?.filter((episode) => {
        const matchesSearch = episode.game_nick
          .toLowerCase()
          .includes(searchTerm.toLowerCase());
        const matchesFilter =
          filterStatus === "all" ||
          episode.status.toLowerCase() === filterStatus.toLowerCase();
    ...
    () => [
      {
        ...
        header: "#",
        cell: ({ row }) => (
          <div className="text-white">{row.getValue("game_episode")}</div>
        ),
      },
      {
          ...
          return (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="unstyled"
                  className="h-8 w-8 p-0 text-white hover:bg-[#ff00ff]/10"
                >
              ...
              <DropdownMenuContent className="bg-[#2a1a35] border-[#ff00ff]/20 text-white">
                ...
                <DropdownMenuItem className="hover:bg-[#ff00ff]/10">
                  <Link
                    href={`/admin/${episode.game_episode}`}
    ...
    return (
      <div className="min-h-screen text-white">
        <div className="flex flex-col items-center justify-center p-12">
          ...
          <Button
            className="mt-4 bg-[#6f2da8] hover:bg-[#8a3ad3] text-white"
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <div className="text-sm text-white/70">
            Dashboard / Episodes / Season {season_id}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
            <DialogTrigger asChild>
              <Button className="bg-[#6f2da8] hover:bg-[#8a3ad3] text-white">
                Create New Episode
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-[#2a1a35] border-[#ff00ff]/20 text-white">
              <DialogHeader>
                <DialogTitle className="text-xl text-[#ff9500]">
                  Create New Episode
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4 mt-4">
                ...
                <div>
                  <Label className="text-sm text-white/70">Game Type</Label>
                  <Select
                    value={episodeType ?? ""}
                    onValueChange={(value) => setEpisodeType(value)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select game type" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="AUDITION_PLAY">Audition</SelectItem>
                      <SelectItem value="STUDIO_GAME">Studio</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex justify-end gap-2 pt-4">
                  ...
                  <Button
                    ...
                    disabled={
                      createEpisodeMutation.isLoading || !newEpisodeName.trim()
                    }
                  >
