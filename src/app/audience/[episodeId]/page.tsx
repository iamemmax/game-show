...
const HustleBoard = () => {
  ...
  // Map local steps to universal steps for heartbeat
  const mapLocalStepToUniversalStep = (localStep: number): UniversalGameStep => {
    switch (localStep) {
      case 1:
        return UNIVERSAL_GAME_STEPS.STAGE1_HUSTLE_PICK
      case 2:
        return UNIVERSAL_GAME_STEPS.STAGE1_HUSTLE_REVEAL
      case 3:
        return UNIVERSAL_GAME_STEPS.STAGE1_QUESTIONS
      case 4:
        return UNIVERSAL_GAME_STEPS.STAGE2_QUESTIONS
      case 5:
        return UNIVERSAL_GAME_STEPS.STAGE3_PICKS_START
      case 6:
        return UNIVERSAL_GAME_STEPS.STAGE4_RAFFLE
      default:
        return UNIVERSAL_GAME_STEPS.GAME_SETUP
    }
  }
  ...
  //   if (step) {
  //     setCurrentUniversalStep(mapLocalStepToUniversalStep(step))
  //   }
  // }, [step])
    ...
    try {
      ...
      if (stored) {
        ...
        if (parsed.step >= 1 && parsed.step <= 6) {
          console.log(`Using saved step ${parsed.step} for episode ${gameEpisode}`)
          return parsed.step
        }
      }
    } catch (error) {
  ...
  return (
    <div className="h-full relative">
      ...
      <div className="absolute top-4 left-4 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-xs z-50">
        Step: {step} | Universal: {currentUniversalStep} | Episode: {gameEpisode}
      </div>
