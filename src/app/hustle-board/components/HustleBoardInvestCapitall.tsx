...
const HustleBoardInvestCapitall = ({ hustleReveal }: Prop) => {
  ...
  // Sort contestants by their actual balance (highest to lowest)
  const sortedContestants = React.useMemo(() => {
    if (!hustleReveal?.data || !allContestsant?.data) return [];
    
    return [...hustleReveal.data].sort((a, b) => {
      const contestantA = getContestantInfo(a.contestant_id);
      const contestantB = getContestantInfo(b.contestant_id);
      
      const balanceA = Number(contestantA?.actual_balance || 0);
      const balanceB = Number(contestantB?.actual_balance || 0);
      
      return balanceB - balanceA; // Descending order (highest to lowest)
    });
  }, [hustleReveal?.data, allContestsant?.data]);

  // Auto-reveal cards one by one - FASTER
  useEffect(() => {
    if (!sortedContestants || sortedContestants.length === 0) return;

    const allCards: string[] = [];
    sortedContestants.forEach((contestant, contestantIdx) => {
      contestant.reveals?.forEach((card, cardIdx) => {
        allCards.push(`${contestantIdx}-${cardIdx}`);
      });
    });
  ...
  return (
    <div className="flex flex-col gap-2 w-full mx-auto px-4">
      {sortedContestants.map((contestant, contestantIdx) => (
        <motion.div 
          key={contestant.contestant_id} 
          className="grid grid-cols-[1fr_5fr] items-start gap-3"
          initial={{ opacity: 0, y: 20 }}
