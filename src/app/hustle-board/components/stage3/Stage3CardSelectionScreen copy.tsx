...
//   // Updated global timer with callback support
//   const [globalTimer, setGlobalTimer] = useState<{
//     show: boolean
//     value: number
//     message: string
...
//   // Helper function to start miss flip timer
//   const startMissFlipTimer = (contestantId: number) => {
//     console.log(`⏰ Starting miss flip timer for contestant ${contestantId} - 2 extra flips`)
//     setMissFlipTimers((prev) => ({ ...prev, [contestantId]: { flipsRemaining: 2, timer: 10 } }))
//     const interval = setInterval(() => {
...
//   // Updated startGlobalTimer with callback support
//   const startGlobalTimer = (seconds: number, message: string, nextPlayer: string, onComplete?: () => void) => {
//     console.log(`⏰ Starting global timer: ${seconds}s - ${message}`)
...
//     setGlobalTimer({ show: true, value: seconds, message, nextPlayer, onComplete })
...
//           // Execute callback when timer completes
//           if (prev.onComplete) {
//             console.log(`⏰ Timer completed, executing callback`)
//             prev.onComplete()
//           }
...
//   // Enhanced Global Timer Component
//   const GlobalTimer = () => {
//     if (!globalTimer.show || globalTimer.value <= 0) return null
...
//   // MQTT message handler with synchronized timer and turn switching
//   useEffect(() => {
//     if (!isConnected) return
...
//             case CARD_TYPES.DUD:
//               console.log(`🎯 DUD card revealed, starting synchronized turn switch`)
//               setRevealedCardInfo({
//                 type: card_type,
//                 index: card_index,
//                 playerName: contestant_name || getContestantName(contestant_id)?.name || "Player",
...
//                 if (otherContestant) {
//                   // Start timer with callback to switch turns when timer completes
//                   startGlobalTimer(1, "Turn switching...", otherContestant.name, () => {
//                     console.log(`🎯 Timer completed, switching turn to ${otherContestant.id}`)
//                     setCurrentTurn(otherContestant.id)
//                   })
//                 }
//               }, 3000)
...
//                 if (otherContestantForMiss) {
//                   // Start timer with callback to switch turns and start miss flip timer
//                   startGlobalTimer(
//                     5,
//                     `${otherContestantForMiss.name} gets 2 extra flips!`,
//                     otherContestantForMiss.name,
//                     () => {
//                       console.log(`🎯 Timer completed, switching to miss flip mode for ${otherContestantForMiss.id}`)
//                       setCurrentTurn(otherContestantForMiss.id)
//                       startMissFlipTimer(otherContestantForMiss.id)
//                     },
//                   )
...
//                 return { ...prev, [contestant_id]: { ...current, flipsRemaining: current.flipsRemaining - 1 } }
//               } else {
//                 // No more extra flips, switch turns with synchronized timer
//                 const otherContestant = remainingContestants.find((c) => c.id !== contestant_id)
//                 if (otherContestant) {
//                   startGlobalTimer(2, "Turn switching back...", otherContestant.name, () => {
//                     setCurrentTurn(otherContestant.id)
//                   })
//                 }
//                 const newState = { ...prev }
