...
const useSoundEffects = () => {
  ...
  const preloadSounds = useCallback(() => {
    ...
    const audio1 = new Audio(SOUND_EFFECTS.elimination);
    audio1.preload = 'auto';
    audio1.volume = 0.8;
    audioRefs.current['elimination'] = audio1;
    
    const audio2 = new Audio(SOUND_EFFECTS.success);
    audio2.preload = 'auto';
    audio2.volume = 0.6;
    audioRefs.current['success'] = audio2;
    
    const audio3 = new Audio(SOUND_EFFECTS.dramatic);
    audio3.preload = 'auto';
    audio3.volume = 0.8;
    audioRefs.current['dramatic'] = audio3;
  }, []);
  ...
  return { preloadSounds, playSound, stopSound, stopAllSounds };
};
...
const HustleBoardStageTallyPage = ({
  ...
  // Sound effects hook
  const { preloadSounds, playSound, stopSound, stopAllSounds } = useSoundEffects();
  ...
  }, [allContestsant?.data, user?.contestant_id, playSound]);
  ...
  const handleItemReveal = useCallback((index: number, isEliminated: boolean) => {
    if (!revealedItems.has(index)) {
      ...
      if (isEliminated) {
        ...
        playSound(`reveal-${soundIndex}`);
      }
    }
  }, [revealedItems, playSound]);
  ...
  }, [revealedItems.size, sortedContestants.length, playSound]);
  ...
  }, [isConnected, onMessage, stopAllSounds]);
